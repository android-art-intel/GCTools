...
dummyDriverSetEnd (dummy_driver_t *dummyDriver,
                   char *end) {
  int offset = offsetof(dummy_driver_tile_t, usedSpace);
  int currSize, totalSize = 0;
  char *start = dummyDriver->area.start;
  int startIndex = gcspy_d_utils_get_index(&dummyDriver->area, start);
  int endIndex = gcspy_d_utils_get_index(&dummyDriver->area, end - 1);
  int *addr = (int *) gcspy_d_utils_get_stats_struct(&dummyDriver->area, startIndex, offset);
  for (int i = startIndex; i <= endIndex; ++i) {
    if ((i == startIndex) && (i == endIndex))
      currSize = end - start;
    else if (i == startIndex) {
      currSize = (size_t) gcspy_d_utils_get_addr(&dummyDriver->area, i + 1) - (size_t) start;
    } else if (i == endIndex) {
      currSize = (size_t) end - (size_t) gcspy_d_utils_get_addr(&dummyDriver->area, i);
    } else {
      currSize = dummyDriver->area.blockSize - random()% dummyDriver->area.blockSize + 1;
    }

    *addr += currSize;
    totalSize += currSize;

    addr = (int *) gcspy_d_utils_next_stats_struct(&dummyDriver->area, addr);
  }
...
dummyDriverInit (dummy_driver_t *dummyDriver,
  ...
  gcspy_driverInit(gcDriver, -1, name, "RosAlloc Driver",
